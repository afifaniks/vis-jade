...
public class AdminAgent extends Agent {
    ...
    protected void setup() {
        ...
        addBehaviour(new CyclicBehaviour() {
             ...
             */
            private String relayRequest(AgentActionIdentifier request) throws IOException, OntologyException, Codec.CodecException {
                ...
                if (Objects.equals(request.getTargetAgent(), AgentIdentifier.AUTHENTICATION)) {
                    ...
                    myAgent.getContentManager().fillContent(message, new Action(new AID(AgentIdentifier.CUSTOMER_ASSISTANT, AID.ISLOCALNAME), action));
                    ...
                    if (request.getAction().equals("subscribe")) {
                        SubscribePackageRequest packageRequest = gson.fromJson(request.getContents(), SubscribePackageRequest.class);
                        User user = new User();
                        user.setEmail(packageRequest.getUserEmail());
                        action = new SubscribePackage(new InsurancePackage(packageRequest.getPackageId()), user, new Vehicle(packageRequest.getVehicleId(), packageRequest.getUserEmail()));
                    }
                    if (request.getAction().equals("vehicle-registration")) {
                        VehicleRegistrationRequest vehicleRegistrationRequest = gson.fromJson(request.getContents(), VehicleRegistrationRequest.class);
                        action = new VehicleRegistration(new User("", vehicleRegistrationRequest.getUserEmail()), new Vehicle(vehicleRegistrationRequest.getUserEmail(), vehicleRegistrationRequest.getVehicleName(), vehicleRegistrationRequest.getVehicleModel(), vehicleRegistrationRequest.getVehicleType(), vehicleRegistrationRequest.getLicenseNumber(), vehicleRegistrationRequest.getVehicleRegistrationNumber(), vehicleRegistrationRequest.getPurchaseDate(), vehicleRegistrationRequest.getVehicleStatus(), vehicleRegistrationRequest.getMileage()));
                    }
                    if (request.getAction().equals("get-package")) {
                        PackageRecommendationRequest packageRequest = gson.fromJson(request.getContents(), PackageRecommendationRequest.class);
                        User user = new User();
                        user.setEmail(packageRequest.getUserEmail());
                        action = new PackageRecommendation(user, new Vehicle(packageRequest.getVehicleId(), packageRequest.getUserEmail()));
                    }
             ...
             * @throws Codec.CodecException If there is any issue with the CODEC.
             */
            private String responseProcessor(ACLMessage responseMessage) throws OntologyException, Codec.CodecException {
                ...
                if (contentElement instanceof SubscriptionSuccess) {
                    ...
                    return gson.toJson(new AgentOperationStatusSchema(200, "Claim successful for user: " + claimSuccess.getUser().getEmail()));
                } else if (contentElement instanceof SignupSuccess signupSuccess) {
                    return gson.toJson(new AgentOperationStatusSchema(signupSuccess.getStatus(), signupSuccess.getMessage()));
                    ...
                    return gson.toJson(new AgentOperationStatusSchema(systemError.getStatus(), systemError.getMessage()));
                }
