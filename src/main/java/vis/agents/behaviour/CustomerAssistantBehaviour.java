...
public class CustomerAssistantBehaviour extends CyclicBehaviour {
    ...
    private void subscribePackage(Action actionContent, ACLMessage responseMessage) throws Codec.CodecException, OntologyException, UnreadableException, IOException {
        ...
        SubscriptionRequestSchema subscriptionRequestSchemaRequest = new SubscriptionRequestSchema(subscribePackage.getUser().getEmail(), subscribePackage.getVehicle().getVehicleId(), subscribePackage.getInsurancePackage().getId());

        SubscriptionStatusSchema subscriptionStatus = customerAssistantService.subscribePackage(subscriptionRequestSchemaRequest);
